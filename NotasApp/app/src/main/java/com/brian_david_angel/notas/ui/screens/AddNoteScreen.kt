...
fun CamposTextoCuerpo(
    ...
    itemUiState: AddNoteViewModel.ItemUiState,
    onItemValueChange: (AddNoteViewModel.ItemDetails) -> Unit
){
    Column(
        modifier = Modifier
            .fillMaxSize()
...
@Composable
fun ItemInputForm(
    itemDetails: AddNoteViewModel.ItemDetails,
    onValueChange: (AddNoteViewModel.ItemDetails) -> Unit = {},
){
    TextField(
...
fun GreetingPreview2() {
    NotasTheme {
        CamposTextoCuerpo(itemUiState = AddNoteViewModel.ItemUiState(
            AddNoteViewModel.ItemDetails(
                titulo = "Item name", contenido = "10.00"
            )
        ), onItemValueChange = {})
    }
}